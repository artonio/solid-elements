export default class DomHandler {
	...
	static getBrowserLanguage() {
		return navigator.language || (navigator.languages && navigator.languages.length && navigator.languages[0]) || navigator.language || 'en';
	}
	...
	static getOuterHeight(el: any, margin?: any) {
		if (el) {
	...
	static getClientHeight(el: any, margin?: any) {
		if (el) {
	...
	static getClientWidth(el: any, margin: any) {
		if (el) {
	...
	static addMultipleClasses(element: any, className: any) {
	...
	static removeMultipleClasses(element: any, className: any) {
		if (element && className) {
			if (element.classList) {
				let styles = className.split(' ');
	...
	static find(element: any, selector: any) {
		return element ? Array.from(element.querySelectorAll(selector)) : [];
	}

	static findSingle(element: any, selector: any) {
		if (element) {
	...
	static alignOverlay(overlay: any, target: any, appendTo: any, calculateMinWidth = true) {
		if (overlay && target) {
			if (appendTo === 'self') {
	...
	static absolutePosition(element: any, target: any) {
		if (element && target) {
	...
	static relativePosition(element: any, target: any) {
		if (element && target) {
			let elementDimensions: any = element.offsetParent ? { width: element.offsetWidth, height: element.offsetHeight } : this.getHiddenElementDimensions(element);
	...
	static flipfitCollision(element: any, target: any, my = 'left top', at = 'left bottom', callback: any) {
		if (element && target) {
			const targetOffset = target.getBoundingClientRect();
			const viewport = this.getViewport();
			const myArr = my.split(' ');
			const atArr = at.split(' ');
			const getPositionValue = (arr: any, isOffset?: any) => (isOffset ? +arr.substring(arr.search(/(\+|-)/g)) || 0 : arr.substring(0, arr.search(/(\+|-)/g)) || arr);
			const position = {
				my: {
	...
	static getHiddenElementDimensions(element: any) {
		let dimensions: any = {};
	...
	static fadeIn(element: any, duration: any) {
		if (element) {
	...
	static fadeOut(element: any, duration: any) {
		if (element) {
			let opacity = 1,
				interval = 50,
	...
	static scrollInView(container: any, item: any) {
	...
	static getFocusableElements(element: any, selector = '') {
		let focusableElements: any = DomHandler.find(
			element,
	...
	static getFirstFocusableElement(element: any, selector?: any) {
		const focusableElements = DomHandler.getFocusableElements(element, selector);

		return focusableElements.length > 0 ? focusableElements[0] : null;
	}

	static getLastFocusableElement(element: any, selector: any) {
		const focusableElements = DomHandler.getFocusableElements(element, selector);
	 ...
	 */
	static focusFirstElement(el: any, scrollTo: any) {
		if (!el) return;
	...
	static invokeElementMethod(element: any, methodName: any, args: any) {
	...
	// static exportCSV(csv: any, filename: any) {
	 ...
	 */
	static getAttributeNames(node: any) {
		let index, rv, attrs;
	 ...
	 */
	static isEqualElement(elm1: any, elm2: any) {
