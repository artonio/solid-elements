export default class ObjectUtils {
	 ...
	 */
	static reduceKeys(obj: any, startsWiths: any) {
		const result: any = {};
		...
		Object.keys(obj)
			.filter((key) => startsWiths.some((value: any) => key.startsWith(value)))
	...
	static findIndexInList(value: any, list: any, dataKey: any) {
		if (list) {
			return dataKey ? list.findIndex((item: any) => this.equals(item, value, dataKey)) : list.findIndex((item: any) => item === value);
		}
	...
	static getProp(props: any, prop = '', defaultProps: any = {}) {
		const value = props ? props[prop] : undefined;

		return value === undefined ? defaultProps[prop] : value;
	}

	static getMergedProps(props: any, defaultProps: any) {
		return Object.assign({}, defaultProps, props);
	}

	static getDiffProps(props: any, defaultProps: any) {
	...
	static getComponentProp(component: any, prop = '', defaultProps = {}) {
		return this.isNotEmpty(component) ? this.getProp(component.props, prop, defaultProps) : undefined;
	}

	static getComponentProps(component: any, defaultProps: any) {
		return this.isNotEmpty(component) ? this.getMergedProps(component.props, defaultProps) : undefined;
	}

	static getComponentDiffProps(component: any, defaultProps: any) {
		return this.isNotEmpty(component) ? this.getDiffProps(component.props, defaultProps) : undefined;
	}

	static isValidChild(child: any, type: any, validTypes: any) {
		/* eslint-disable */
		if (child) {
			...
			try {
				if (process.env.NODE_ENV !== 'production' && !isValid) {
					...
					console.error(
						`PrimeReact: Unexpected type; '${childType}'. Parent component expects a ${messageTypes.map((t: any) => `${t}`).join(' or ')} component or a component with the ${messageTypes
							.map((t: any) => `__TYPE="${t}"`)
	...
	static sort(value1: any, value2: any, order = 1, locale: any, nullSortOrder = 1) {
	...
	static compare(value1: any, value2: any, locale: any, order = 1) {
		let result = -1;
